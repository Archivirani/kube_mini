...
export class PatientServicesComponent implements OnInit, OnDestroy {
  ...
  constructor(public clinicService: ClinicService, private dataService: DataService, private router: Router, private modalService: BsModalService,private documentationService: DocumentationService) {
   this.SaveAndPrint()
        ...
        if (this.createClinicForm.valid) {
          ...
          if (medicationData && medicationData.length) {
            ...
            const order = {
              ...
              doctor_code: this.appointmentDetails?.specialities?.doctors?.doctor_Code,
