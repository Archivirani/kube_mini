...
import { appointmentPieChartData, conversionChartData } from './charts';
...
export class DashboardComponent {
  ...
  public todayAppointmentsBackup: Appointment[];
  ...
  constructor(public dataService: DataService, private chartService: ChartService , public resourceService:ResourceService,public globalSearch:GlobalSearchFilter,private cdr: ChangeDetectorRef) {
    window.innerWidth < 1260 ? this.isIPadAir = true : this.isIPadAir = false;
    this.chartDataConfig = this.chartService;
    if (this.searchSubscription) { this.searchSubscription.unsubscribe(); }
    this.searchSubscription=this.resourceService.searchQuerySubject.subscribe((res) => {
      const fieldNames = ["patient.medicalRecordNumber","patient.contactNo1","appointmentType.name","appointmentDateTime","patient.dateOfBirth","patient.gender","patient.familyName","patient.thirdName","patient.secondName","patient.firstName","specialities.speciality_desc", "doctors.doctor_Name"];
      this.todayAppointments = this.globalSearch.transform(this.todayAppointmentsBackup, res, fieldNames);
    });
  }
  getAppointmentToday(date: Date) {
    ...
    this.dataService.getData<Appointment[]>(`${Modules.AppointmentByDate}/${formatDate(date, "YYYY-MM-ddTHH:mm:ss", "en-Us")}`).then((response) => {
      ...
      if (response && response.length) {
        response.forEach((element) => { element.patient.profileUrl = !!element.patient.profileUrl ? `${Modules.Images}${sessionStorage.TenantCode}/Images/${element.patient.profileUrl}` : "" }); 
        // this.todayAppointments = response;
        // this.todayAppointmentsBackup=response;
        this.todayAppointments = response.filter(appointment => appointment?.appointmentType?.name !== 'walk-in');
        this.todayAppointmentsBackup = this.todayAppointments;
      }
    });
  ...
  onloadAppointmentCount(data: string, date?: Date) {
    ...
    switch (data) {
      case 'Day':
        this.fromDay = `${formatDate(new Date(new Date().setDate(new Date().getDate())), "YYYY-MM-dd", "en-Us")}T00:00:00`;
