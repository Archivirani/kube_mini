...
@Component({
  selector: 'services-clinic-order',
  templateUrl: './services-clinic-order.component.html',
  styleUrls: ['./services-clinic-order.component.scss']
})
export class ServicesClinicOrderComponent implements OnInit {
  showPatients = false;
 ...
 async SaveandPrint(){
   ...
   this.orderSetSubscription = this.clinicService.orderSet.subscribe(async (resp) => {
     ...
     await    this.dataService.get(`${Modules.getServiceHistory}/${this.appointmentDetails.patientId}`).then((response:any) => {
            ...
            const unpaidOrderServices = ordersDataService.filter(service => !service.isPaid);
            const combinedData = unpaidOrderServices.concat(orderServices)
